...
export function SkeletonLoader({
  ...
  if (variant === 'spinner') {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        ...
        {message && (
          <div className="mt-4">
            <p className="text-lg font-medium text-gray-300 dark:text-gray-700">
              {message}
            </p>
          </div>
        )}
      ...
      <div className="animate-pulse">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="h-24 bg-gray-700 dark:bg-gray-200 rounded-lg"
            ></div>
          ))}
        </div>
          ...
          {[...Array(rows)].map((_, i) => (
            <div key={i} className="grid grid-cols-3 gap-4">
              {[...Array(cols)].map((_, j) => (
                <div
                  key={j}
                  className="h-10 bg-gray-700 dark:bg-gray-200 rounded"
                ></div>
              ))}
            </div>
          ))}
    ...
    return (
      <div className="animate-pulse space-y-8">
        <div className="max-w-2xl mx-auto text-center mb-12">
          <div className="h-8 bg-gray-700 dark:bg-gray-200 rounded-full w-48 mx-auto mb-4"></div>
          <div className="h-4 bg-gray-700 dark:bg-gray-200 rounded w-full max-w-md mx-auto"></div>
          {showStopwatch && (
            <div className="mt-4">
              <Stopwatch />
            </div>
  ...
  return (
    <div className="animate-pulse flex flex-col items-center justify-center py-8 space-y-4 text-center">
      <div className="h-4 bg-gray-700 dark:bg-gray-200 rounded w-48"></div>
      {message && (
