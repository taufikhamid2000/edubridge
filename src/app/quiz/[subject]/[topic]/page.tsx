...
// Main topic page component using App Router params
export default function TopicPage({ params }: TopicPageProps) {
  const routerParams = useParams();
  const router = useRouter();

  // Use params from props or from useParams() hook as fallback
  const subject = params?.subject || (routerParams?.subject as string);
  const topic = params?.topic || (routerParams?.topic as string);
