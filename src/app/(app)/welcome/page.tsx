...
// This page holds EduBridge's original marketing / pitch-deck content
// (previously the public root "/" homepage). The root route now gates on
// auth and redirects; this page is preserved and linked from About so the
// content isn't lost. It is reachable by anyone (logged in or not) who
// wants to read the full pitch.
export default function WelcomePage() {
  const router = useRouter();
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [isParallaxEnabled, setIsParallaxEnabled] = useState(false);

  // Listen for auth changes so the "Access your dashboard" CTA below can
  // route logged-in visitors straight to /dashboard instead of /auth.
  useEffect(() => {
    const {
      data: { subscription },
  ...
  return (
    <>
      <Head>
        <title>EduBridge - Vision & Pitch</title>
      </Head>{' '}
